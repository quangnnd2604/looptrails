...
**Appears on:** home, tours-archive, tour-detail (`hgl-grid hgl-grid-2`)
**Measured dimensions:** Desktop: 397×68px, radius 0, gap 16px, 4 items — this exact geometry (397×68, gap 16) is identical across all three templates at the desktop breakpoint, a strong cross-template match per the grouping rule. Tablet: 311–397×66-70px, gap 16px. Mobile/narrow-mobile: single-column stack (negative gapX).
...
**Spec cross-reference:** §5.2 "transport option cards"; §5.4 "bus transfers and add-ons".
...
**Measured dimensions:** No `accommodation`/`hotel`/`homestay`/`stay`-related class names, typography entries or card-grid geometry appear anywhere in `docs/design-measurements/` (checked across all 53 files). No dedicated "Accommodation" tab content was captured in any screenshot (screenshots record only the default-active tab state of the Home tabbed section, and no other template surfaces this component).
