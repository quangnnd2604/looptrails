...
Based on the same screenshots and the shared Elementor element ID `elementor-element-50aa`, which appears on Home, tour-detail, about, and contact's footer background color entries (`#e4e0da`) — again indicating one shared/global footer template part reused sitewide.
